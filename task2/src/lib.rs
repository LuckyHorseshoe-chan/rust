...
mod tests {
    ...
    fn inter_transanction() {
        ...
        assert_eq!(*bank.clients[0].balance.read().unwrap(), 150);
        assert_eq!(*bank.clients[1].balance.read().unwrap(), 150);
    }

    #[test]
    fn extra_transanction() {
        let (tx_bank1, rx_bank1) = mpsc::channel::<Transaction>();
        ...
        send_transaction(bank.clone(), tx1.clone(), tx_bank2_copy.clone(), transaction1);
        bank2 = rx2.recv().unwrap();
        bank = rx1.recv().unwrap();
        send_transaction(bank2.clone(), tx2.clone(), tx_bank1_copy.clone(), transaction2);
        bank = rx1.recv().unwrap();
        bank2 = rx2.recv().unwrap();

        assert_eq!(*bank.clients[0].balance.read().unwrap(), 190);
        assert_eq!(*bank2.clients[0].balance.read().unwrap(), 210);
    }
    ...
    fn transanctions() {
        ...
        assert_eq!(*bank.clients[0].balance.read().unwrap(), 30);
        assert_eq!(*bank.clients[1].balance.read().unwrap(), 160);
        assert_eq!(*bank2.clients[0].balance.read().unwrap(), 410);
    }

    #[test]
    fn not_allowed_transanction() {
        let (tx_bank1, rx_bank1) = mpsc::channel::<Transaction>();
        ...
        assert_eq!(*bank.clients[0].balance.read().unwrap(), 100);
    }
}